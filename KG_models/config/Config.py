...
class Config(object):
	...
	'''
	def __init__(self,log_path,model_name,pre_trained):
		...
		self.logger = self.get_logger(self.model_name, self.log_path)
		self.pre_trained = pre_trained
		self.fingerprint_path = '/dataset/virtual_screening/compounds/compound_fp_dicts.pkl'
		self.entity_path = '/home/junyoung/workspace/MOA/dataset3/entity2id.txt'
	...
	def get_logger(self, logger_name, logger_path):
		...
		with self.graph.as_default():
			with self.sess.as_default():
				...
				for times in range(self.train_times):
					...
					if self.log_on:
						self.logger.warning('[%s|Train|Epoch %d] Loss: %f' % (self.model, times, loss))
						#t_init = time.time()
						#t_end = time.time()
